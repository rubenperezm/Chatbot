  ...
  if(empiezo === 0){
    ...
    if(evt.data.indexOf("ErrorVialaravel") > -1){
      ...
      if(evt.data.indexOf("finalConversacionCorrectolaravel") > -1){
        ...
        if(ejercicio[1] !== 0){
          usandoPromesas(appUrl + "/api/apiEjercicio/show/" + evt.data[3])
          .then( data =>{
...
var ConversationPanel = (function () {
  ...
  function init() {
    ...
    if(ejercicio !== undefined)
      if(ejercicio[1] !== 0){
        usandoPromesas(appUrl + "/api/apiEjercicio/show/" + ejercicio[1])
        .then( data =>{
          var enunciado = JSON.parse(data[0]['enunciado']);
          var ayuda = JSON.parse(data[0]['ayuda']);
          chatUpdateSetup();
...
function displayMessage(newPayload, typeValue) {
  ...
  if (isUser !== null && textExists) {
    ...
    if(newPayload.input && newPayload.input.text.indexOf("laravel") > -1 && typeValue == settings.authorTypes.user) isLaravel = false;
      ...
      var responses = buildMessageDomElements(newPayload, isUser,isLaravel);
...
function setResponse(responses, isUser, chatBoxElement, index, isTop, isLaravel) {
  if(isLaravel){
    ...
    var theUrl = appUrl;
    if(ejercicio[1] !== 0) 
      theUrl = theUrl + "/api/apiEjercicio/storeConversacion";
    else
    theUrl = theUrl + "/api/apiModoLibre/storeConversacion";
...
function sendMessage(text, fromLaravelAyuda = "",fromLaravelComprobar = new Array()) {
  ...
  if(fromLaravelComprobar.length > 0){
    ...
    for (var i = 0; i < fromLaravelComprobar.length; i++) {
      if(i == 0){
        context['skills']['main skill']['user_defined'].comprobarClausula = fromLaravelComprobar[i];
      }else{
        if(i+1 !== fromLaravelComprobar.length)  context['skills']['main skill']['user_defined'].comprobarClausula = context['skills']['main skill']['user_defined'].comprobarClausula + ", " + fromLaravelComprobar[i];
        else  context['skills']['main skill']['user_defined'].comprobarClausula = context['skills']['main skill']['user_defined'].comprobarClausula + " y " + fromLaravelComprobar[i];
      }
    }
  }
